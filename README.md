...
개인 운영 메모는 멤버 리스트와 유저 시트 상세 화면에서 동일하게 조회하고 수정할 수 있습니다.

## 데이터 저장과 권한

- 참가 명단, 참여 대조 내용과 팀 결과는 로그인한 Discord 계정별로 현재 브라우저에 24시간 저장됩니다.
- 유저 시트는 Upstash Redis에 저장되어 등록된 관리자들이 함께 사용합니다.
- 개인 운영 메모는 로그인한 관리자 ID와 BattleTag 조합으로 분리되어 작성자에게만 보입니다.
