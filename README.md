...
운영진이 함께 사용하는 유저 시트, 개인·공유 메모, 참여 대조와 결과 이미지 복사도 지원합니다.
...
비선호 역할은 한 명당 하나만 허용됩니다. 비선호가 두 개 이상이거나 형식을 읽지 못한
항목은 임의로 보정하지 않고 자동 추가에서 제외합니다. 입력 영역에 표시된 항목을 누르면
수동 입력으로 옮겨 수정할 수 있습니다.
...
`이미지로 복사`를 누른 뒤 Discord 채팅에 바로 붙여넣습니다. 유저 시트에 등록된 특이사항도
BattleTag가 일치하면 대진표에 함께 표시됩니다.
...
두 메모는 목적과 공개 범위가 다릅니다.

| 종류 | 공개 범위 | 용도 |
| --- | --- | --- |
| 특이사항 | 모든 관리자 | 대진표에도 표시할 공통 유저 정보 |
| 나만 보기 | 작성한 관리자 본인 | 개인적으로 참고할 운영 메모 |
| 관리자 공유 | 모든 관리자 | 대진표에는 표시하지 않는 운영진 내부 메모 |

개인·공유 운영 메모는 멤버 리스트와 유저 시트 상세 화면에서 동일하게 조회하고 수정할 수 있습니다.

## 데이터 저장과 권한

- 참가 명단, 참여 대조 내용과 팀 결과는 로그인한 Discord 계정별로 현재 브라우저에 24시간 저장됩니다.
- 유저 시트와 관리자 공유 메모는 Upstash Redis에 저장되어 등록된 관리자들이 함께 사용합니다.
- 개인 메모는 로그인한 관리자 ID와 BattleTag 조합으로 분리되어 작성자에게만 보입니다.
